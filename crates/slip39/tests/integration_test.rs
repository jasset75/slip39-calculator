...
fn test_wordlist_matches_official_slip39() {
    ...
    let response =
        reqwest::blocking::get(OFFICIAL_URL).expect("Failed to download official SLIP-39 wordlist");

    let official_text = response.text().expect("Failed to read response text");
    ...
    println!(
        "✓ Wordlist matches official SLIP-39 specification ({} words)",
        words.len()
    );
}
